...
if [[ "$TEST_MODE" != 1 ]]; then
  [[ "$APP_ROOT" == /opt/outflow && "$DATA_DIR" == /var/lib/outflow \
     && "$CONFIG_FILE" == /etc/outflow/outflow.env ]] \
...
[[ ! -e "$release_dir" ]] || outflow_die "Release directory already exists: $release_dir"
...
if [[ "$TEST_MODE" == 1 ]]; then
  if [[ "$FAIL_AT" == health ]]; then rollback; outflow_die 'Simulated health-check failure'; fi
else
