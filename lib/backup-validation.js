...
function validateBackupSemantics(backup) {
  ...
  const schedulesById = new Map(backup.income_schedules.map(row => [String(row.id), row]));
  const historicalSchedulesBySeries = new Map();
  for (const row of backup.income) {
    const occurrence = occurrencesById.get(String(row.recurring_occurrence_id));
    if (!occurrence || row.source_schedule_id == null) continue;
    const key = String(occurrence.series_id);
    if (!historicalSchedulesBySeries.has(key)) historicalSchedulesBySeries.set(key, new Set());
    historicalSchedulesBySeries.get(key).add(String(row.source_schedule_id));
  }
  ...
  for (const series of backup.recurring_series) {
    ...
    const historicalScheduleIds = historicalSchedulesBySeries.get(id);
    const historicalSchedule = historicalScheduleIds?.size === 1
      ? schedulesById.get([...historicalScheduleIds][0]) : null;
    const validHistoricalIncomeEntity = series.kind === 'income' && !schedulesBySeries.has(id)
      && ['deleted', 'completed'].includes(series.status)
      && historicalSchedule && String(historicalSchedule.user_id) === String(series.user_id);
    ...
    const expectedEntity = (series.kind === 'bill' && billsBySeries.has(id))
      ...
      || validHistoricalIncomeEntity
      || (series.kind === 'transaction' && transactionsBySeries.has(id))
      || (series.kind === 'transfer' && transfersBySeries.has(id));
    const expectedEntityCount = validHistoricalIncomeEntity ? 0 : 1;
    if (entityCount !== expectedEntityCount || !expectedEntity) {
  ...
  for (const [table, kind] of destinationTables) {
    for (const row of backup[table]) {
      ...
      if (kind === 'income' && occurrence.scheduled_date !== row.date) {
        recordError(errors, `income ${row.id} date does not match its recurring occurrence`);
      }
  ...
  const orphanChecks = [
    ['recurring series entities', `SELECT COUNT(*) AS count FROM recurring_series s
      ...
      WHERE (s.kind = 'bill' AND b.id IS NULL) OR (s.kind = 'income' AND i.id IS NULL
        AND NOT (s.status IN ('deleted', 'completed')
          AND EXISTS (SELECT 1 FROM recurring_occurrences history_occurrence
            JOIN income history_income ON history_income.recurring_occurrence_id = history_occurrence.id
            JOIN income_schedules history_schedule ON history_schedule.id = history_income.source_schedule_id
            WHERE history_occurrence.series_id = s.id AND history_schedule.user_id IS s.user_id)
          AND NOT EXISTS (SELECT 1 FROM recurring_occurrences mixed_occurrence
            JOIN income first_income ON first_income.recurring_occurrence_id = mixed_occurrence.id
            JOIN recurring_occurrences other_occurrence ON other_occurrence.series_id = s.id
            JOIN income other_income ON other_income.recurring_occurrence_id = other_occurrence.id
            WHERE mixed_occurrence.series_id = s.id
              AND first_income.source_schedule_id IS NOT other_income.source_schedule_id)))
