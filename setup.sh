...
if [[ "$TEST_MODE" == 1 ]]; then outflow_secure_release_tree "$release_dir" "$SERVICE_USER" 0
else outflow_secure_release_tree "$release_dir" "$SERVICE_USER" 1; fi
outflow_atomic_link "$release_dir" "$APP_LINK"
touch "$APP_ROOT/.outflow-installation"
install_complete=1

if [[ "$TEST_MODE" != 1 ]]; then
  chown root:"$SERVICE_USER" "$APP_ROOT" "$RELEASES_DIR" "$APP_ROOT/.outflow-installation"
  chown -R "$SERVICE_USER:$SERVICE_USER" "$DATA_DIR" "$LOG_DIR"
  chown root:"$SERVICE_USER" "$CONFIG_DIR"
  chmod 750 "$APP_ROOT" "$RELEASES_DIR" "$CONFIG_DIR"
  chown root:"$SERVICE_USER" "$CONFIG_FILE"
  chmod 640 "$CONFIG_FILE"
fi

env OUTFLOW_TEST_MODE="$TEST_MODE" OUTFLOW_SYSTEMD_PROFILE="$SYSTEMD_PROFILE" \
  OUTFLOW_SYSTEMD_UNIT_DIR="$UNIT_DIR" OUTFLOW_SERVICE_USER="$SERVICE_USER" \
  OUTFLOW_DEFER_START="$DEFER_START" bash "$SCRIPT_DIR/scripts/install-outflow-service.sh"
